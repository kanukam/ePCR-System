...
export default class Settings extends Component {
    ...
    componentDidMount() {
        ...
        const url = `http://localhost:3000/api/users/${this.context.username}`;
        const options = {
            method: 'GET',
            headers: {
    ...
    handleUpdate = (event => {
        ...
        console.log(phone);
        if (name && phone && email) {
            const url = `http://localhost:3000/api/users/${this.context.username}/update`;
            const options = {
                method: 'POST',
        ...
        if (oldPassword && newPassword && confirmedPassword) {
            ...
            if (newPassword === confirmedPassword) {
                if (this.passwordWordCheck(newPassword)) {
                    const url = `http://localhost:3000/api/users/${this.context.username}/password`;
                    const options = {
                        method: 'POST',
